...
@php
  $colspan = 3 + intval($max_installment);
  $totalMajor = 0;
@endphp
  ...
  <body>
    <table class="no-border">
      <tbody>
        ...
        <tr><td colspan="{{ $colspan + 1 }}" align="center" class="title"><b>LAPORAN PEMBAYARAN PER KELOMPOK CALON SANTRI <br/> {{ $payloads->payment }}</b></td></tr>
        <tr><td colspan="{{ $colspan + 1 }}" align="center" class="subtitle"><b>DEPARTEMEN {{ $payloads->department }} - KELOMPOK {{ $payloads->prospect_group }}</b></td></tr>
      </tbody>
    </table>
    <br/>
    <table border="1" cellpadding="2" style="border-collapse: collapse;overflow:wrap;" width="100%">
      <thead>
        <tr style="background-color:#CCFFFF;">
          ...
          <th class="subtitle">No.Registrasi</th>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{ $pay->student }}</td>
          @php $receipts = $receiptVoluntaryEloquent->listPaymentProspectGroup($payloads->department_id, $payloads->prospect_group_id, $pay->prospect_student_id); @endphp
