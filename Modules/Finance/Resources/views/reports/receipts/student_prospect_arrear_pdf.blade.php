...
<html>
  <head>
    <title>{{ config('app.name') .' '. strtoupper(Session::get('institute')) }} - LAPORAN PEMBAYARAN PER CALON SANTRI MENUNGGAK</title>
  ...
  <body>
    ...
    <div id="body">
      <br/>
      <div class="text-center" style="font-size:16px;"><b>LAPORAN PEMBAYARAN PER CALON SANTRI MENUNGGAK - {{ $requests->payment }}</b></div>
      <br/>
      <br/>
      <div>
        <table class="table no-border" style="font-size: 13px;font-weight:700">
          <tbody>
            ...
            <tr>
              <td style="width:15%;">Proses</td>
              <td style="width:1%;text-align:center;">:</td>
              <td>{{ $requests->admission }}</td>
            </tr>
            <tr>
              <td style="width:15%;">Kelompok</td>
              <td style="width:1%;text-align:center;">:</td>
              <td>{{ $requests->prospect_group }}</td>
            </tr>
          </tbody>
        </table>
        <br/>
        <table class="table" style="width:100%;">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $pay->student }}</td>
              @php 
                $receipts = $receiptMajorEloquent->listPaymentProspectGroup($requests->department_id, $pay->prospect_student_id, $requests->status); 
              @endphp
              ...
              @endfor
              @endif
              <td class="text-center">{{ $receiptMajorEloquent->paymentProspectDelay($requests->department_id, $category->id, $requests->payment_id, $requests->duration, $requests->date_delay, $pay->prospect_student_id)->pluck('delay')->first() }}</td>
