...
<div class="container-fluid mt-1 mb-1">
    <div class="row">
        <div class="col-8 pl-2">
            <h6>Data Pembayaran: <br/><b>{{ $requests['student_no'] }} - {{ $requests['student'] }}</b></h6>
        </div>
        <div class="col-4 p-2 text-right">
            <a class="easyui-linkbutton top-btn" data-options="iconCls:'ms-Icon ms-Icon--PDF'" onclick="exportReportPaymentStudent('pdf')">Ekspor PDF</a>
            <a class="easyui-linkbutton top-btn" data-options="iconCls:'ms-Icon ms-Icon--ExcelDocument'" onclick="exportReportPaymentStudent('excel')">Ekspor Excel</a>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    function exportReportPaymentStudent(document) {
       var payload = {
       		is_prospect: 0,
        ...
        exportDocument("{{ url('finance/report/receipt/student/export-') }}" + document,payload,"Ekspor data Laporan ke "+ document.toUpperCase(),"{{ csrf_token() }}")
    }
</script>