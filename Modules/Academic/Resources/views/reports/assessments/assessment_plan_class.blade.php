...
<div style="overflow-y: auto;">
    <div class="container-fluid mt-1 mb-1">
        <div class="row">
            ...
            <div class="col-12">
                <form id="form-report-avg-lesson-plan">
                <input type="hidden" id="id-report-avg-lesson-plan-deptid" />
                <input type="hidden" id="id-report-avg-lesson-plan-semester" />
                <input type="hidden" id="id-report-avg-lesson-plan-lesson" />
                <input type="hidden" id="id-report-avg-lesson-plan" />
                <input type="hidden" id="id-report-avg-lesson-plan-exam" />
                <input type="hidden" id="id-report-avg-lesson-plan-name" />
                ...
                <span class="mr-2"></span>
                <a href="javascript:void(0)" class="easyui-linkbutton lbtn" onclick="filterAcademicReportAssessmentLessonStat(1)" style="height:22px;" data-options="iconCls:'ms-Icon ms-Icon--Search'"></a>
                <a href="javascript:void(0)" class="easyui-linkbutton lbtn" onclick="$('#form-report-avg-lesson-plan').form('reset');filterAcademicReportAssessmentLessonStat(-1)" style="height:22px;" data-options="iconCls:'ms-Icon ms-Icon--Clear'"></a>
                </form>
            </div>
            <div class="col-3">
                <br/>
                <div id="dl-avg-lesson-plan" class="easyui-datalist" title="RPP" lines="true" style="width:100%;height:{{ $GridHeight }}"></div>
            </div>
            <div class="col-9">
                <br/>
                    ...
                    <div class="container-fluid">
                        <div class="row">
                            ...
                            <div class="col-3 text-right">
                                <a href="javascript:void(0)" class="easyui-linkbutton lbtn" iconCls="ms-Icon ms-Icon--PDF" plain="true" onclick="exportAcademicReportAssessmentLessonStat('pdf')">Ekspor PDF</a>
                            </div>
                            <div class="col-4 pr-2">
                                <table id="tb-avg-lesson-plan-data" style="width:100%;height:{{ $GridHeightSub }};" data-options="">
                                    <thead>
                                        <tr>
...
<script type="text/javascript">
    $(function () {
        var dgavgplan = $("#tb-avg-lesson-plan-data").datagrid()
        $("#AcademicReportGrade").combogrid({
            url: '{{ url('academic/grade/combo-grid') }}',
            queryParams: { _token: "{{ csrf_token() }}" },
            onClickRow: function(index, row) {
                $("#id-report-avg-lesson-plan-deptid").val(row.department_id)
                $("#id-report-avg-lesson-plan-semester").val(row.semester_id)
        ...
        $("#dl-avg-lesson-plan").datalist({
            onDblClickRow: function(index,row) {
                var rpps = row.value.split("-")
                $("#id-report-avg-lesson-plan-name").val(row.text)
                $("#id-report-avg-lesson-plan").val(rpps[0])
                $("#id-report-avg-lesson-plan-lesson").val(rpps[1])
        ...
        $("#AcademicReportAssessmentType").combobox({
            ...
            onSelect: function(record) {
                $("#loader-graph-report").css("visibility","visible")
                $("#id-report-avg-lesson-plan-exam").val(record.value)
                $.post("{{ url('academic/report/assessment/plan/average/class/graph') }}", { 
                    _token: "{{ csrf_token() }}",  
                    semester_id: $("#id-report-avg-lesson-plan-semester").val(),
                    lesson_exam_id: record.value,
                    lesson_plan_id: $("#id-report-avg-lesson-plan").val(),
                    lesson_id: $("#id-report-avg-lesson-plan-lesson").val(),
    ...
    function filterAcademicReportAssessmentLessonStat(val) {
        if (val > 0) {
            $("#dl-avg-lesson-plan").datalist("reload", "{{ url('academic/report/assessment/plan/average/class/data') }}" 
                + "?_token=" + "{{ csrf_token() }}" 
                + "&department_id=" + $("#id-report-avg-lesson-plan-deptid").val()
                + "&semester_id=" + $("#id-report-avg-lesson-plan-semester").val()
                + "&grade_id=" + $("#AcademicReportGrade").combobox("getValue")
                + "&lesson_id=" + $("#AcademicReportLesson").combobox("getValue")
            )
        } else {
            $("#dl-avg-lesson-plan").datalist("loadData", [])
            $("#tb-avg-lesson-plan-data").datagrid("loadData", [])
            $("#AcademicReportAssessmentGraph").html("")
            $("#AcademicReportAssessmentType").combobox("loadData", [])
            $("#AcademicReportAssessmentType").combobox("clear")
        }
    }
    function exportAcademicReportAssessmentLessonStat(document) {
        var dg = $("#tb-avg-lesson-plan-data").datagrid('getData')
        if (dg.total > 0) {
            var payload = {
                ...
                grade_id: $("#AcademicReportGrade").combobox("getValue"),
                lesson_exam_id: $("#id-report-avg-lesson-plan-exam").val(),
                lesson_plan_id: $("#id-report-avg-lesson-plan").val(),
                lesson_id: $("#id-report-avg-lesson-plan-lesson").val(),
                lesson: $("#AcademicReportLesson").combobox("getText"),
                lesson_exam: $("#AcademicReportAssessmentType").combobox("getText"),
                lesson_plan: $("#id-report-avg-lesson-plan-name").val(),
            }
            exportDocument("{{ url('academic/report/assessment/plan/average/class/export-') }}" + document,payload,"Ekspor Rata-Rata RPP Kelas ke "+ document.toUpperCase(),"{{ csrf_token() }}")
        }
    }
</script>