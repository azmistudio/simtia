...
<div style="overflow-y: auto;">
    <div class="container-fluid mt-1 mb-1">
        <div class="row">
            ...
            <div class="col-12">
                <form id="form-report-score-legger">
                <input type="hidden" id="id-report-score-legger-deptid" />
                <input type="hidden" id="id-report-score-legger-semester" />
                <input type="hidden" id="id-report-score-legger-lesson" />
                <input type="hidden" id="id-report-score-legger" />
                <input type="hidden" id="id-report-score-legger-exam" />
                <input type="hidden" id="id-report-score-legger-name" />
                <input type="hidden" id="id-report-score-legger-schoolyearid" />
                ...
                <span class="mr-2"></span>
                <a href="javascript:void(0)" class="easyui-linkbutton lbtn" onclick="filterAcademicReportScoreLegger(1)" style="height:22px;" data-options="iconCls:'ms-Icon ms-Icon--Search'"></a>
                <a href="javascript:void(0)" class="easyui-linkbutton lbtn" onclick="$('#form-report-score-legger').form('reset');filterAcademicReportScoreLegger(-1)" style="height:22px;" data-options="iconCls:'ms-Icon ms-Icon--Clear'"></a>
                </form>
            </div>
            <div class="col-12">
                <br/>
                <div id="AcademicReportExamScoreLegger" class="easyui-panel pnel" style="width:100%;height:{{ $GridHeight }};padding: 10px;"></div>
            </div>
        </div>
    </div>
</div>
<script type="text/javascript">
    $(function () {
        $("#tb-score-legger-data").datagrid()
        $("#AcademicReportClass").combobox({ readonly : true })
        $("#AcademicReportGrade").combogrid({
            url: '{{ url('academic/grade/combo-grid') }}',
            queryParams: { _token: "{{ csrf_token() }}" },
            onClickRow: function(index, row) {
                $("#id-report-score-legger-deptid").val(row.department_id)
                $("#id-report-score-legger-semester").val(row.semester_id)
                $("#id-report-score-legger-schoolyearid").val(row.schoolyear_id)
    ...
    function filterAcademicReportScoreLegger(val) {
        if (val > 0) {
            $("#AcademicReportExamScoreLegger").panel("refresh", "{{ url('academic/report/assessment/score/legger/view') }}"
                + "?lesson_id=" + $("#AcademicReportLesson").combobox("getValue")
                + "&semester_id=" + $("#id-report-score-legger-semester").val()
            ...
            $("#AcademicReportLesson").combobox("clear")
            $("#AcademicReportExamScoreLegger").panel("clear")
        }
    }
</script>