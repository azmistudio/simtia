...
<div style="overflow-y: auto;">
    <div class="container-fluid mt-1 mb-1">
        <div class="row">
            ...
            <div class="col-12">
                <form id="form-report-score-legger-lesson">
                <input type="hidden" id="id-report-score-legger-lesson-deptid" />
                <input type="hidden" id="id-report-score-legger-lesson-semester" />
                <input type="hidden" id="id-report-score-legger-lesson-lesson" />
                <input type="hidden" id="id-report-score-legger-lesson" />
                <input type="hidden" id="id-report-score-legger-lesson-exam" />
                <input type="hidden" id="id-report-score-legger-lesson-name" />
                <input type="hidden" id="id-report-score-legger-lesson-schoolyearid" />
                ...
                <span class="mr-2"></span>
                <a href="javascript:void(0)" class="easyui-linkbutton lbtn" onclick="filterAcademicReportScoreLeggerLesson(1)" style="height:22px;" data-options="iconCls:'ms-Icon ms-Icon--Search'"></a>
                <a href="javascript:void(0)" class="easyui-linkbutton lbtn" onclick="$('#form-report-score-legger-lesson').form('reset');filterAcademicReportScoreLeggerLesson(-1)" style="height:22px;" data-options="iconCls:'ms-Icon ms-Icon--Clear'"></a>
                </form>
            </div>
            <div class="col-12">
                <br/>
                <div id="AcademicReportExamScoreLeggerLesson" class="easyui-panel pnel" style="width:100%;height:{{ $GridHeight }};padding: 10px;"></div>
            </div>
        </div>
    </div>
</div>
<script type="text/javascript">
    $(function () {
        $("#tb-score-legger-lesson-data").datagrid()
        $("#AcademicReportClass").combobox({ readonly : true })
        $("#AcademicReportGrade").combogrid({
            url: '{{ url('academic/grade/combo-grid') }}',
            queryParams: { _token: "{{ csrf_token() }}" },
            onClickRow: function(index, row) {
                $("#id-report-score-legger-lesson-deptid").val(row.department_id)
                $("#id-report-score-legger-lesson-semester").val(row.semester_id)
                $("#id-report-score-legger-lesson-schoolyearid").val(row.schoolyear_id)
    ...
    function filterAcademicReportScoreLeggerLesson(val) {
        if (val > 0) {
            $("#AcademicReportExamScoreLeggerLesson").panel("refresh", "{{ url('academic/report/assessment/score/legger/lesson/view') }}"
                + "?lesson_id=" + $("#AcademicReportLesson").combobox("getValue")
                + "&semester_id=" + $("#id-report-score-legger-lesson-semester").val()
                + "&class_id=" + $("#AcademicReportClass").combobox("getValue")
                + "&department=" + $("#AcademicReportDepartment").textbox("getText")
                + "&schoolyear_id=" + $("#id-report-score-legger-lesson-schoolyearid").val()
            ...
            $("#AcademicReportLesson").combobox("clear")
            $("#AcademicReportExamScoreLeggerLesson").panel("clear")
        }
    }
</script>