...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        $view = "CREATE OR REPLACE VIEW academic.class_students_view AS (
                    SELECT academic.classes.id, schoolyear_id, academic.classes.grade_id, UPPER(class) as class, capacity || '/' || COUNT(students.class_id) as capacity,
                        ...
                        JOIN ( SELECT id, department_id, semester, grade_id FROM academic.semesters WHERE is_active = 1) AS semesters ON academic.classes.grade_id = semesters.grade_id
                        WHERE academic.classes.is_active = 1 AND academic.schoolyears.is_active = 1
                        GROUP BY academic.classes.id, schoolyear_id, academic.classes.grade_id, class, public.departments.id, public.departments.name, academic.schoolyears.school_year, academic.grades.grade, semesters.id, semesters.semester, academic.schoolyears.is_active
