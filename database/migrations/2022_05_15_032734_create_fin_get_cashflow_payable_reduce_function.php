...
return new class extends Migration
{
    ...
    public function up()
    {
        $procedure = "  CREATE OR REPLACE FUNCTION finance.fn_get_cashflow_payable_reduce (
                        ...
                        AS $$
                            ...
                            BEGIN
                                ...
                                WHERE c.category = 'HARTA' AND a.credit > 0 AND a.journal_id IN (
                                    ...
                                    WHERE e.source <> 'begin_balance' AND e.journal_date BETWEEN p_date_start AND p_date_end AND e.bookyear_id = p_bookyear_id AND g.category = 'HUTANG'
                                    AND d.debit > 0
                                ) INTO v_value;
                                RETURN v_value;
                            END;
    ...
    {
        DB::unprepared("DROP FUNCTION IF EXISTS finance.fn_get_cashflow_payable_reduce"); 
    }
};
