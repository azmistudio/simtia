...
return new class extends Migration
{
    ...
    public function up()
    {
        $procedure = "  CREATE OR REPLACE FUNCTION finance.fn_get_cashflow_equities_withdrawal (
                            ...
                            BEGIN
                                FOR var_r IN(
                                    ...
                                    (
                                        ...
                                        AND f.journal_date BETWEEN p_date_start AND p_date_end AND f.bookyear_id = p_bookyear_id AND h.category = 'MODAL' AND e.debit > 0
                                    ) AS j
                                    WHERE a.account_id = b.id AND b.category_id = c.id AND a.journal_id = d.id and j.journal_id = a.journal_id AND a.credit > 0 AND c.category = 'HARTA'
                                    GROUP BY j.name
                                ) LOOP
                                    name := var_r.name;
                                    value := var_r.value;
    ...
    {
        DB::unprepared("DROP FUNCTION IF EXISTS finance.fn_get_cashflow_equities_withdrawal"); 
    }
};
