...
return new class extends Migration
{
    ...
    public function up()
    {
        $procedure = "  CREATE OR REPLACE FUNCTION finance.fn_get_cashflow_receivables (
                        ...
                        AS $$
                            ...
                            BEGIN
                                FOR var_r IN(
                                    ...
                                    WHERE a.account_id = b.id AND b.category_id = c.id AND c.category = 'HARTA' AND a.credit > 0 
                                    AND a.journal_id IN (
                                        SELECT d.journal_id
                                        ...
                                        AND g.category = 'HARTA' AND f.code LIKE '1-2%' AND d.debit > 0
                                    )
                                    GROUP BY b.name
                                ) LOOP
                                    value := var_r.value;
    ...
    {
        DB::unprepared("DROP FUNCTION IF EXISTS finance.fn_get_cashflow_receivables"); 
    }
};
